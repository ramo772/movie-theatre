                ...
                <div class="card mb-4 mx-4">
                    <div class="card-header pb-0">
                        <div class="d-flex flex-row justify-content-between">
                            <div>
                                <h5 class="mb-0">All Event Days</h5>
                            </div>
                        ...
                        <x-modal formid="Scat" id="create-modal" method="post" action="{{ route('day.store') }}">
                            <x-slot name="header">Add New Event Day</x-slot>
                            <x-slot name="body">
                                @include('admin.forms.create-day')
                            </x-slot>
                        </x-modal>
                    </div>
                                        ...
                                        <tr>
                                            ...
                                            <td class="ps-4">
                                                <p class="text-xs font-weight-bold mb-0">
                                                    {{ \Carbon\Carbon::parse($row->day)->format('M d,  Y') }}</p>
                                            </td>

                                            <td class="text-center">
                                                <a class="fas fa-eye text-secondary" type="button"
                                                    href="{{route('day.show',$row->id)}}">
                                                </a>
                                            ...
                                            <x-modal id="edit-modal-{{ $row->id }}" method="put"
                                                action="{{ route('day.update', $row->id) }}">
                                                <x-slot name="header">Edit {{ $row->name }} movie</x-slot>
                                                <x-slot name="body">
                                                    @include('admin.forms.edit-day')
                                                </x-slot>
                                            </x-modal>

                                            <x-modal-delete id="delete-modal-{{ $row->id }}" method="delete"
                                                action="{{ route('day.destroy', ['day' => $row->id]) }}">
                                                <x-slot name="header">you will delete the day {{ $row->day }} with its
